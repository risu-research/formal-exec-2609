...
import argparse, hashlib, json, math, os, re, resource, shutil, subprocess, tempfile, time
...
def parse_maxrss(text):
    for ln in text.splitlines():
        if "Maximum resident set size (kbytes):" in ln:return int(ln.rsplit(":",1)[1].strip())
    return None

def timed_cmd(cmd):
    tf=tempfile.NamedTemporaryFile(delete=False);tf.close();t0=time.perf_counter_ns()
    p=subprocess.run(["/usr/bin/time","-v","-o",tf.name,*cmd],text=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    ms=(time.perf_counter_ns()-t0)/1e6;txt=Path(tf.name).read_text(errors="replace");os.unlink(tf.name)
...
def z3_run_text(text):
    s=z3.Solver();s.from_string(text);return str(s.check())
...
def bench_real_queries(refroot,cvc5):
    ...
    for p in sorted(Path(refroot).rglob("*.smt2")):
        rel=str(p.relative_to(refroot)).replace(os.sep,"/");text=p.read_text();zr=z3_run_text(text);cr=cvc5_run(cvc5,p)
        if zr!=cr:raise RuntimeError(f"solver warmup mismatch {rel}: {zr}/{cr}")
        zt=[];ct=[]
        for _ in range(REPS):
...
def skeleton(A,H):
    decl=[f"(declare-fun x_{t}_{i} () Bool)" for t in range(H+1) for i in range(A)]
    trans=[f"(= x_{t+1}_{i} (xor x_{t}_{i} x_{t}_{(i+1)%A}))" for t in range(H) for i in range(A)]
    return decl,"(and "+" ".join(trans)+")"

def emit_query(A,H,family,direction,path):
    decl,F=skeleton(A,H);x="x_0_0"
    if family=="STRICT_EXPANSION":old=f"(and {F} (not {x}))";new=F;expected="sat" if direction=="new_not_old" else "unsat"
    else:old=F;new=f"(and {F} (or {x} (not {x})) (or x_0_{A-1} (not x_0_{A-1})))";expected="unsat"
    q=f"(and {new} (not {old}))" if direction=="new_not_old" else f"(and {old} (not {new}))"
    text="(set-logic ALL)\n"+"\n".join(decl)+f"\n(assert {q})\n(check-sat)\n";Path(path).write_text(text)
    return {"expected":expected,"variables":A*(H+1),"bytes":len(text.encode())}
      ...
      for H in HORIZONS:
       for fam in ("STRICT_EXPANSION","EQUALITY"):
        for direction in ("new_not_old","old_not_new"):
            p=qdir/f"a{A}_h{H}_{fam}_{direction}.smt2";meta=emit_query(A,H,fam,direction,p);text=p.read_text();z0=z3_run_text(text);c0=cvc5_run(cvc5,p)
...
def slope(v1,t1,v2,t2):
    return math.log(t2/t1)/math.log(v2/v1) if t1>0 and t2>0 and v2>v1>0 else None
    ...
    for fam in ("STRICT_EXPANSION","EQUALITY"):
      for direction in ("new_not_old","old_not_new"):
       for axis,levels,fixed_key,fixed in (("atoms",ATOMS,"horizon",64),("horizon",HORIZONS,"atoms",1024)):
        sub=[next(r for r in rows if r[axis]==lv and r[fixed_key]==fixed and r["family"]==fam and r["direction"]==direction) for lv in levels]
        ts=[r[solver]["p50"] for r in sub];vs=[r["unrolled_boolean_variables"] for r in sub];ratio=ts[-1]/ts[0];sl=slope(vs[0],ts[0],vs[-1],ts[-1]);mono=all(ts[i+1]>ts[i] for i in range(3));fired=ratio>=10 and sl is not None and sl>1.25 and mono
...
def main():
    ...
    for rep in range(REPS):
        ...
        if sha_file(p0/"g6-r0-corpus-manifest.json")!=CORPUS_SHA:raise RuntimeError("P0 identity mismatch")
        stages["P0_HISTORY_CORPUS"].append(m)
        ...
        if sha_file(p1/"g6-r1-inventory.json")!=R1_SHA:raise RuntimeError("P1 identity mismatch")
        stages["P1_EXTRACT_NORMALIZE"].append(m)
        ...
        if query_map(p2/"queries")!=refmap:raise RuntimeError("P2 exact query-byte map mismatch")
        stages["P2_CLASSIFY_QUERY_BUILD"].append(m)
    stage_summary={}
    for k,rr in stages.items():stage_summary[k]={"wall_ms":stats([x["wall_ms"] for x in rr]),"maxrss_kib":stats([x["maxrss_kib"] for x in rr if x["maxrss_kib"] is not None]),"repetitions":rr}
    ...
    real_rows,real_summary=bench_real_queries(ref/"queries",a.cvc5);ladder,qdir=bench_ladder(out,a.cvc5);t2=eval_t2(ladder,"z3")+eval_t2(ladder,"cvc5");t2f=[x for x in t2 if x["T2_fired"]]
    t3=[]
    for r in ladder:
        for s in ("z3","cvc5"):
