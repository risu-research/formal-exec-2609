#!/usr/bin/env python3
"""E5 S1: no-cover pin-level SMT check of exact child RTL under one assumption intervention."""
import hashlib
import json
...
PIN = ('f_cyc','f_outstanding','i_lock','o_wb_stb_gbl','o_wb_stb_lcl','o_wb_cyc_gbl','o_wb_cyc_lcl','o_busy')

def sha(raw): return hashlib.sha256(raw).hexdigest()
def v(state,name): return f'(|pipemem_n {name}| {state})'
def no(s): return f'(not {s})'
def endpoint(s):
    return f'(and {no(v(s,"f_cyc"))} (= {v(s,"f_outstanding")} #b00000) {no(v(s,"o_wb_stb_gbl"))} {no(v(s,"o_wb_stb_lcl"))} (or {v(s,"o_wb_cyc_gbl")} {v(s,"o_wb_cyc_lcl")}))'

def build(model,mode):
    for stem in ('pipemem_i','pipemem_t','pipemem_u','pipemem_h','pipemem_is'):
        assert '|'+stem+'|' in model,stem
    for name in PIN: assert '|pipemem_n '+name+'|' in model,name
    ...
    for i in range(HORIZON+1):
        ...
        lines.append(f'(assert {"(|pipemem_is| "+s+")" if i==0 else no("(|pipemem_is| "+s+")")})')
        lines.append(f'(assert (|pipemem_u| {s}))')
        if i==0: lines.append(f'(assert (|pipemem_i| {s}))')
        if i: lines.append(f'(assert (|pipemem_t| |q{i-1}| {s}))')
    if mode=='pin_any':
        query='(or '+' '.join(endpoint(f'|q{i}|') for i in range(HORIZON))+')'
    elif mode=='observed_cycle':
        # Post-hoc trace-replay query, not an independently preselected endpoint.
        query='(and '+' '.join((v('|q2|','f_cyc'),no(v('|q2|','i_lock')),v('|q3|','f_cyc'),v('|q3|','i_lock'),endpoint('|q4|'),no(v('|q5|','o_wb_cyc_gbl'))))+')'
    elif mode=='negative_control':
        query='(and '+endpoint('|q4|')+' '+no(v('|q4|','o_wb_cyc_gbl'))+' '+v('|q4|','o_wb_cyc_gbl')+')'
    else: raise ValueError(mode)
    lines.extend(['(assert '+query+')','(check-sat)'])
    if mode=='observed_cycle':
        lines.append('(get-value ('+' '.join(v(f'|q{i}|',name) for i in (2,3,4,5) for name in PIN)+'))')
    lines.append('(exit)')
    return '\n'.join(lines)+'\n'

def main():
    summary={'schema':'e5-s1-uninstrumented-pin-check-v1','horizon':HORIZON,'posthoc_replay':True,'cases':{}}
    ...
    for arm in ('E_removed','E_restored'):
        ...
        for mode in ('pin_any','observed_cycle','negative_control'):
            q=build(model,mode)
