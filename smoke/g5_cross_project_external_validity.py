...
def dual_direction(pred, qpath: Path, cvc5: str):
    ...
    if zres == "sat":
        ...
        fixed = {"query_relpath": str(fixed_path), "query_sha256": sha256_bytes(fixed_text.encode()), "cvc5": fixed_cvc5}
    return {"query_relpath": str(qpath), "query_sha256": qsha, "z3": zres, "cvc5": cres,
            "complete_witness": witness, "fixed_replay": fixed}
...
def p1_semantics():
    mv = z3.Bool("p1_mem_valid_t0"); mr = z3.Bool("p1_mem_ready_t0")
    pmv = z3.Bool("p1_mem_valid_tm1"); pmr = z3.Bool("p1_mem_ready_tm1")
    rd = z3.BitVec("p1_mem_rdata", 32); insn = z3.BitVec("p1_monitor_insn", 32)
    ms = z3.Bool("p1_monitor_state"); cyc = z3.BitVec("p1_cycle", 8)
    curr = z3.Or(z3.Not(mv), mr); prev = z3.Or(z3.Not(pmv), pmr)
    fairness = z3.Or(curr, prev); insn_eq = rd == insn
    base_e = z3.And(fairness, insn_eq); strict_e = z3.And(base_e, curr)
    ...
    T = z3.BoolVal(True)
    return {
        "base": (base_e, prop), "strict": (strict_e, prop), "redundant": (z3.And(base_e, insn_eq), prop),
        "macro_off": (base_e, prop), "macro_on": (strict_e, prop), "no_guarantee": (base_e, T),
        "rewrite": (base_e, rewrite), "strict_no_guarantee": (strict_e, T), "comment": (base_e, prop),
        "alpha": (base_e, prop), "combined": (base_e, combined), "split": (base_e, split),
        "aux": (base_e, prop), "design_commute": (base_e, prop),
    }


def p2_semantics():
    s0 = z3.BitVec("p2_src_sel_t0", 8); s1 = z3.BitVec("p2_src_sel_t1", 8)
    t0 = z3.BitVec("p2_tgt_sel_t0", 1); t1 = z3.BitVec("p2_tgt_sel_t1", 1)
    ip0 = z3.Bool("p2_ip_sel_t0"); ip1 = z3.Bool("p2_ip_sel_t1"); claim0 = z3.Bool("p2_claim_sel_t0")
    ia = z3.BitVec("p2_gateway_ia", 184); ipv = z3.BitVec("p2_gateway_ip", 184)
    ...
    active = (ia | ~ipv) != z3.BitVecVal(0, 184); active_rewrite = (~ipv | ia) != z3.BitVecVal(0, 184)
    base_g = z3.And(ipclear, active); no_ipclear = active; T = z3.BoolVal(True)
    return {
        "base": (base_e, base_g), "strict": (strict_e, base_g),
        "redundant": (z3.And(base_e, tgt_range(t0), tgt_range(t1)), base_g),
        "macro_off": (T, base_g), "macro_on": (base_e, base_g), "no_ipclear": (base_e, no_ipclear),
        "rewrite": (base_e, z3.And(ipclear, active_rewrite)), "strict_no_ipclear": (strict_e, no_ipclear),
        "comment": (base_e, base_g), "alpha": (base_e, base_g), "combined": (base_e, z3.And(ipclear, active)),
        "split": (base_e, z3.And(ipclear, active)), "aux": (base_e, base_g), "design_commute": (base_e, base_g),
    }
...
def classify_project(project: str, sem, pairs, expected, out: Path, cvc5: str):
    ...
    for cid, (oldn, newn) in pairs.items():
        olde, oldg = sem[oldn]; newe, newg = sem[newn]
        ...
        overall = overall_from_axes(elab, glab); exp = expected[cid]
        rows.append({
            ...
            "environment_relation": {"raw": eraw, **edetail}, "guarantee_relation": {"raw": graw, **gdetail},
            "pass": (elab, glab, overall) == exp,
        })
    return rows
...
def structural_rows(project: str):
    if project == "P1":
        details = {
            "A08":{"old_selected":["honest","complete"],"new_selected":["honest"]},
            "A09":{"old_selected":["honest"],"new_selected":["honest","complete"]},
            "A10":{"old_depth":30,"new_depth":20},
            "N05":{"old_engine":"abc bmc3","new_engine":"smtbmc z3","old_depth":30,"new_depth":30},
            "N06":{"old_selected":["honest","complete"],"new_selected":["complete","honest"]},
            "N07":{"old_depth":20,"new_depth":30},
        }
        calc = {"A08":set_relation(details["A08"]["old_selected"],details["A08"]["new_selected"]),
                "A09":set_relation(details["A09"]["old_selected"],details["A09"]["new_selected"]),
                "A10":frontier_relation(30,20),"N05":"R_EQ",
                "N06":set_relation(details["N06"]["old_selected"],details["N06"]["new_selected"]),"N07":frontier_relation(20,30)}
        expected = P1_STRUCT_EXPECTED
    else:
        details = {
            "A08":{"old_selected":["IpClearAfterClaim_A","ActiveIfPending_A"],"new_selected":["IpClearAfterClaim_A"]},
            "A09":{"old_selected":["IpClearAfterClaim_A"],"new_selected":["IpClearAfterClaim_A","ActiveIfPending_A"]},
            "A10":{"old_horizon":4,"new_horizon":3},
            "N05":{"old_solver":"z3","new_solver":"cvc5","old_horizon":4,"new_horizon":4},
            "N06":{"old_selected":["IpClearAfterClaim_A","ActiveIfPending_A"],"new_selected":["ActiveIfPending_A","IpClearAfterClaim_A"]},
            "N07":{"old_horizon":3,"new_horizon":4},
        }
        calc = {"A08":set_relation(details["A08"]["old_selected"],details["A08"]["new_selected"]),
                "A09":set_relation(details["A09"]["old_selected"],details["A09"]["new_selected"]),
                "A10":frontier_relation(4,3),"N05":"R_EQ",
                "N06":set_relation(details["N06"]["old_selected"],details["N06"]["new_selected"]),"N07":frontier_relation(3,4)}
        expected = P2_STRUCT_EXPECTED
    rows=[]
    for cid in ("A08","A09","A10","N05","N06","N07"):
        exp_rel, exp_overall = expected[cid]; rel = calc[cid]
        overall = "REGRESSION_PRESENT" if rel in {"TASK_DROP","R_WEAKEN"} else "NONREGRESSION"
        rows.append({"id":f"G5-{project}-{cid}","detail":details[cid],"computed_relation":rel,"expected_relation":exp_rel,
...
def p1_sources(riscv: Path, pico: Path, work: Path):
    honest_path = riscv / "cores/picorv32/honest.sv"; sby_path = riscv / "cores/picorv32/honest.sby"
    complete_path = riscv / "cores/picorv32/complete.sby"; macros_path = riscv / "checks/rvfi_macros.vh"; pico_path = pico / "picorv32.v"
    honest = honest_path.read_text(); sby = sby_path.read_text(); complete = complete_path.read_text(); macros = macros_path.read_text(); pico_text = pico_path.read_text()
    fairness = "\t\tassume((!mem_valid || mem_ready) || $past(!mem_valid || mem_ready));"
    instr = "\t\tassume (mem_rdata == monitor_insn);"; prop = "\t\tif (!monitor_state) assert (cycle < 21);"
    ...
    require(alpha != honest and re.search(r"\bmonitor_state\b", alpha) is None, "P1 alpha rename incomplete")
    ...
    audit={"repository_revision":RISCV_FORMAL_SHA,"dependency_revision":PICORV32_SHA,"honest_sha256":sha256_file(honest_path),
           "honest_sby_sha256":sha256_file(sby_path),"complete_sby_sha256":sha256_file(complete_path),
           "rvfi_macros_sha256":sha256_file(macros_path),"picorv32_sha256":sha256_file(pico_path),"anchors_verified":True}
    return variants, macros, audit
...
def run_p1_yosys(variants, macros: str, work: Path):
    ...
    for name,(honest,pico,macro_on) in variants.items():
        d=work/"p1-yosys"/name; d.mkdir(parents=True,exist_ok=True)
        (d/"rvfi_macros.vh").write_text(macros); (d/"picorv32.v").write_text(pico); (d/"honest.sv").write_text(honest); outj=d/"elaborated.json"
        ...
        if p.returncode==0 and outj.exists():
            j=json.loads(outj.read_text()); top=j["modules"]["testbench"]; types=[c.get("type") for c in top.get("cells",{}).values()]
...
def p2_source_audit(open_titan: Path, out: Path):
    ...
    mp=open_titan/"hw/ip/prim/rtl/prim_assert.sv"; bp=open_titan/"hw/top_earlgrey/ip_autogen/rv_plic/fpv/tb/rv_plic_bind_fpv.sv"
    ...
    require("parameter int NumSrc = 184;" in p,"P2 Earl Grey NumSrc != 184"); require("parameter int NumTarget = 1;" in p,"P2 Earl Grey NumTarget != 1")
    ...
    for x,n in ((isrc,"IsrcRange"),(itgt,"ItgtRange"),(ipclear,"IpClear"),(active,"ActiveIfPending")): require(a.count(x)==1,f"P2 exact {n} anchor changed")
    ...
    alpha=re.sub(r"\bsrc_sel\b","src_sel_g5",a); require(alpha!=a and re.search(r"\bsrc_sel\b",alpha) is None,"P2 alpha rename incomplete")
    aux=replace_once(g,"\nendmodule\n","\n  logic g5_aux_state;\n  always_ff @(posedge clk_i or negedge rst_ni) begin\n    if (!rst_ni) g5_aux_state <= 1'b0;\n    else g5_aux_state <= ~g5_aux_state;\n  end\n\nendmodule\n","P2 aux state")
    d_old="ip_o <= (ip_o & ~claim_i) | (set & ~ia);"; d_new="ip_o <= (set & ~ia) | (ip_o & ~claim_i);"
    commute=replace_once(g,d_old,d_new,"P2 N08 design commutation")
    overlay_texts={"base":a,"strict":strict,"redundant":redundant,"no_ipclear":no_ipclear,"rewrite":rewrite,"strict_no_ipclear":strict_no_ipclear,"comment":comment,"alpha":alpha}
    meta={}
    for name,text in overlay_texts.items():
        path=od/f"assert__{name}.sv"; path.write_text(text); meta[name]={"path":str(path),"sha256":sha256_file(path)}
    for name,text in {"base":g,"aux":aux,"design_commute":commute}.items():
        path=od/f"gateway__{name}.sv"; path.write_text(text); meta["gateway_"+name]={"path":str(path),"sha256":sha256_file(path)}
    return {"repository_revision":OPENTITAN_SHA,"assertion_sha256":sha256_file(ap),"gateway_sha256":sha256_file(gp),"macro_sha256":sha256_file(mp),
            "bind_sha256":sha256_file(bp),"package_sha256":sha256_file(pp),"bound_num_src":184,"bound_num_target":1,
            "assume_fpv_count":a.count("`ASSUME_FPV("),"assert_count":a.count("`ASSERT("),"selected_anchors_verified":True,"overlays":meta}


def local_n08_checks(out: Path, cvc5: str):
    compressed,prefetch,rinst,nextpc1,not_second=[z3.Bool(x) for x in ("p1_compressed","p1_prefetch","p1_rinst","p1_nextpc1","p1_not_secondword")]
    old1=z3.And(compressed,z3.Or(prefetch,rinst),nextpc1,not_second); new1=z3.And(compressed,z3.Or(rinst,prefetch),nextpc1,not_second)
    p1=dual_direction(z3.Xor(old1,new1),out/"queries/local-n08/p1_xor.smt2",cvc5); require(p1["z3"]=="unsat","P1 N08 local equivalence failed")
    ...
    p2=dual_direction(old2!=new2,out/"queries/local-n08/p2_neq.smt2",cvc5); require(p2["z3"]=="unsat","P2 N08 local equivalence failed")
    return {"P1":p1,"P2":p2,"pass":True}


def a11_count_controls(p1_yosys, p2_audit):
    m={r["variant"]:r for r in p1_yosys}; p1_old=m["base"]["assume_cells"]+m["base"]["assert_cells"]
    p1_new=m["strict_no_guarantee"]["assume_cells"]+m["strict_no_guarantee"]["assert_cells"]
    p2_old=4+2; p2_new=5+1
    return {"P1":{"old_total":p1_old,"new_total":p1_new,"equal":p1_old==p1_new,
            ...
            "P2":{"old_total":p2_old,"new_total":p2_new,"equal":p2_old==p2_new,
                  ...
                  "source_assume_count":p2_audit["assume_fpv_count"]},
    ...
    for row in rows:
        for axis in ("environment_relation","guarantee_relation"):
            for k in ("old_only","new_only"):
                d=row[axis][k]
                if d["z3"]=="sat": hs.add(d["query_sha256"])
    return sorted(hs)


def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--riscv-formal",required=True); ap.add_argument("--picorv32",required=True)
    ap.add_argument("--opentitan",required=True); ap.add_argument("--cvc5",required=True); ap.add_argument("--work",required=True); ap.add_argument("--out",required=True)
    ns=ap.parse_args(); riscv=Path(ns.riscv_formal).resolve(); pico=Path(ns.picorv32).resolve(); ot=Path(ns.opentitan).resolve()
    ...
    p1vars,macros,p1audit=p1_sources(riscv,pico,work); p1yosys=run_p1_yosys(p1vars,macros,work); write_json(out/"P1_NATIVE_ELABORATION.json",p1yosys)
    p1_native_green=all(r["pass"] for r in p1yosys)
    p2audit=p2_source_audit(ot,out); write_json(out/"P1_SOURCE_AUDIT.json",p1audit); write_json(out/"P2_SOURCE_AUDIT.json",p2audit)
    p1rows=classify_project("P1",p1_semantics(),P1_PAIRS,P1_EXPECTED,out,ns.cvc5); p2rows=classify_project("P2",p2_semantics(),P2_PAIRS,P2_EXPECTED,out,ns.cvc5)
    p1struct=structural_rows("P1"); p2struct=structural_rows("P2"); local=local_n08_checks(out,ns.cvc5)
    counts=a11_count_controls(p1yosys,p2audit) if p1_native_green else {"pass":False,"reason":"P1_NATIVE_RED"}
    all_rows=p1rows+p2rows+p1struct+p2struct; strict_hashes={"P1":unique_strict_query_hashes(p1rows),"P2":unique_strict_query_hashes(p2rows)}
    ...
    result={"schema":"g5-cross-project-external-validity-v1","g5_pre_result_freeze_sha":G5_FREEZE_SHA,"g4_parent_sha":G4_PARENT_SHA,
            ...
            "P1":{"mode":"NATIVE_OPEN_SOURCE_ELABORATION_PLUS_INDEPENDENT_REPLAY","source_audit":p1audit,
                  "native_elaboration":{"variant_count":len(p1yosys),"pass_count":sum(1 for x in p1yosys if x["pass"]),"pass":p1_native_green},"semantic":p1rows,"structural":p1struct},
            "P2":{"mode":"SOURCE_SEMANTICS_ADAPTER_PLUS_INDEPENDENT_REPLAY","native_tool_metadata":"jaspergold","native_jaspergold_execution_performed":False,
                  "source_audit":p2audit,"semantic":p2rows,"structural":p2struct},
            "n08_local_equivalence":local,"a11_equal_count_controls":counts,"strict_sat_query_unique_hashes":strict_hashes,
            "denominator":{"P1":20,"P2":20,"total":40,"observed_rows":len(all_rows)},"failures":failures,"public_gate_green":gate_green,
    ...
    if not gate_green: raise SystemExit(2)
